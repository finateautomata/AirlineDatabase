...
# app = Flask(__name__, template_folder='/Users/feiteng/Library/Mobile Documents/com~apple~CloudDocs/2021.1 Spring/Databases/Project/ProjectPart3/DB_Project_Final', static_folder = '')
app = Flask(__name__, template_folder='/Users/feiteng/Library/Mobile Documents/com~apple~CloudDocs/2021.1 Spring/Databases/Project/ProjectPart3/DB_Project_Final', static_folder = '/Users/feiteng/Library/Mobile Documents/com~apple~CloudDocs/2021.1 Spring/Databases/Project/ProjectPart3/DB_Project_Final/css')
...
def AloginAuth():
    ...
    if(data):
		...
		#session is a built in
        # cursor = conn.cursor()
        # query = 'SELECT booking_agent_id FROM booking_agent WHERE email = %s and password = %s'
        # cursor.execute(query, (email, password))
...
#Authenticates the register for clients
...
#Authenticates the register for agents
...
def AregisterAuth():
    ...
    cursor = conn.cursor()
    query = """
        SELECT max(booking_agent_id) FROM booking_agent
    """
    cursor.execute(query)
    data = cursor.fetchall()
    booking_agent_id = int(data[0]['max(booking_agent_id)']) + 1
    cursor.close()

    print(email, password, booking_agent_id)
    #cursor used to send queries
    cursor = conn.cursor()
    #executes query
...
def agent_home():
    ...
    session['email'] = email
    cursor.close()
    return render_template('agent_home.html', name = email)
...
def view_flights():
    ...
    cursor.close()
    print(data)
    return render_template('agent_my_flights.html', data = data, data2 = data2, name = name)
...
def agent_view_flights():
    ...
    cursor.close()
    print(data)
    return render_template('my_flights.html', data = data, data2 = data2, name = email)
...
def view_top_customers():
    ...
    cursor = conn.cursor()
    query1 = """SELECT customer_email, COUNT(customer_email)
                FROM purchases 
                WHERE booking_agent_id = %s
                GROUP BY customer_email
                ORDER BY count(customer_email) DESC LIMIT 5"""
    cursor.execute(query1, (agent_id))
    data_NumberOfTickets = cursor.fetchall()
    # print(data1)
    processedX_NameOfCustomers = []
    processedY_NumberOfTickets = []
    for i in range(len(data_NumberOfTickets)):
        processedX_NameOfCustomers.append(data_NumberOfTickets[i]['customer_email'])
    for i in range(len(data_NumberOfTickets)):
        processedY_NumberOfTickets.append(int(data_NumberOfTickets[i]['COUNT(customer_email)']))

    cursor.close()

    cursor = conn.cursor()
    queryOfCommission = """
        SELECT customer_email,SUM(commission) 
        FROM purchases NATURAL JOIN booking_agent
        WHERE email = %s
        GROUP BY customer_email
        ORDER BY 2 DESC LIMIT 5
    """
    cursor.execute(queryOfCommission, (email))
    data_AmountOfCommission = cursor.fetchall()
    # print(data_AmountOfCommission)

    print(data_AmountOfCommission)
    processedX_NameOfCustomersInCommission = []
    processedY_AmountOfCommission = []
    for i in range(len(data_AmountOfCommission)):
        processedX_NameOfCustomersInCommission.append(data_AmountOfCommission[i]['customer_email'])
    for i in range(len(data_AmountOfCommission)):
        processedY_AmountOfCommission.append(int(data_AmountOfCommission[i]['SUM(commission)']))

    cursor.close()
    return render_template('agent_view_top_customers.html', dataX_NumberOfTickets = processedX_NameOfCustomers, \
        dataY_NumberOfTickets = processedY_NumberOfTickets, dataX_AmountOfCommission = processedX_NameOfCustomersInCommission, \
        dataY_AmountOfCommission = processedY_AmountOfCommission, agent_id = agent_id)
...
def view_my_commission():
    ...
    cursor = conn.cursor()
    query = """
        SELECT SUM(commission), COUNT(ticket_id), FORMAT(SUM(commission)/ COUNT(ticket_id), 0)
        FROM purchases NATURAL JOIN booking_agent
        WHERE email = %s
    """
    ...
    data = cursor.fetchall()
    print(data)
    return render_template('agent_view_my_commission.html', data = data, email = email)
...
def render_purchase():
    ...
    data = cursor.fetchall()
    # session['ticket_no'] = str(int(['max(ticketID)']) + 1) # here's a bug, the ticket_no is not unique

    session['ticket_no'] = str(int(data[0]['max(ticketID)']) + 1)
    # session['ticket_no'] = str(1)
